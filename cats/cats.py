...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    paragraphs_true = [elem for elem in paragraphs if select(elem)]
    if k < len(paragraphs_true):
        return paragraphs_true[k]
    return ''
    # END PROBLEM 1
...
def about(topic):
    ...
    def select_function(paragraph):
        ...
        return any([x for x in topic if x in preprocessed_text])
    ...
    if typed_words == [] and reference_words == []:
        ...
        for i in range(min(len(typed_words),len(reference_words))):
            if typed_words[i] == reference_words[i]:
                correct_words.append(typed_words[i])
        if correct_words != []:
            return (len(correct_words) / len(typed_words)) *100
        return 0.0
    # END PROBLEM 3
    # Alternative
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    avg_words_length = 5
    return len(typed) / avg_words_length / elapsed * 60
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    #     w_diff.append(diff_function(typed_word, w, limit))
    # if typed_word in valid_words or min(w_diff) > limit:
    #     return typed_word
    # else:
    #     return valid_words[w_diff.index(min(w_diff))]
    # END PROBLEM 5
    # Alternative
    if typed_word in valid_words:
        return typed_word
    closest_word = min(valid_words, key=lambda w: diff_function(typed_word, w, limit))
